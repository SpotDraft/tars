...
class LoggingConfig:
    """dictConfig-compatible structured JSON logging for GCP/Kubernetes.

    Call ``logging.config.dictConfig(LoggingConfig.to_dict())`` once at
    application startup (in ``main.py``) before any loggers are created.
    """
    ...
    formatters: ClassVar[dict] = {
        "json": {
            "()": "app.core.log_config.GcpJsonFormatter",
